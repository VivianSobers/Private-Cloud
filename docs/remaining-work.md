...
**Companion pages.** [status.md](status.md) is the same picture as a grid — every
phase and slice marked ✅ / 🟠 / ❌ — and [deferred-work.md](deferred-work.md) holds
the reason for each deliberate absence. This page is the narrative: what is left and
*what blocks it*. Where they overlap they are meant to agree; if they ever disagree,
the mechanical check is `awaitingClient` in
[contract_test.go](../server/internal/httpapi/contract_test.go), which now names a
single route.
