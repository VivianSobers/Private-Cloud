...
func (s *Server) serveMediaVariant(w http.ResponseWriter, r *http.Request, id uuid.UUID, variant string) {
	...
	// The variant's own identity: which rendition, of which content.
	//
	// It was the storage key, which is a random blob address — the exact detail
	// nodeJSON deliberately withholds, on the grounds that "exposing them would
	// freeze the on-disk layout that Phase 2 is going to replace". One rule, two
	// answers, and the leaking one was in a header cached for a year.
	//
	// The content hash is the better identity as well as the safer one: it is
	// stable across a re-render, so a thumbnail regenerated byte-for-byte
	// identically keeps its cache entry instead of being handed a new random key.
	w.Header().Set("ETag", `"`+variant+"-"+hex.EncodeToString(v.ContentHash)+`"`)
