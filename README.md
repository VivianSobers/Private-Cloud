...
| 9 | Scale & resilience: cold tier, DR automation, quotas | **partial** — see [docs/phase-9-design.md](docs/phase-9-design.md). `GET /admin/storage` reports pool health, backup freshness and queue depth by reading the **same textfile collectors the alerts scrape**, so the console and Grafana cannot disagree; quota enforcement is proven end to end, including that an editor's write into a shared folder spends the *owner's* quota. **The object-storage cold tier is not built**, and the endpoint says `tiering.enabled: false` rather than reporting an empty cold tier that would imply it exists — §1 of the design records what it would take and why half-building it is the worst option for a storage system |
