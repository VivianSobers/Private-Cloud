...
| **3** | Sync engine: change journal, Go client, conflict resolution | **in progress** — slice 1 complete: a per-owner change journal with a gap-free cursor (`GET /changes`), so a client can resume from where it left off or be told to re-sync; delta protocol, Go client and conflict resolution next |
