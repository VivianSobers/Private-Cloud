...
| 4 | ML: OCR, semantic search, tagging; OIDC; hardening | **in progress** — see [docs/phase-4-design.md](docs/phase-4-design.md). Two tiers, one queue: the always-on box owns the API, database and blob store; intelligence runs in a separate `pcworker` that drains the job queue via `SKIP LOCKED`, so GPU workers on separate boxes over the tailnet (or a CPU fallback on the always-on box) do the heavy inference, and a remote worker pulls file bytes over the API rather than a mounted store. Content never leaves your infrastructure. Slices 1 (job queue + worker) ✅, 2 (OCR/text extraction folded into search, so a scanned receipt is findable by a word printed on it) ✅, and 3 (semantic search — a Python embedding sidecar on a GPU box, content-addressed vectors, brute-force cosine KNN, `GET /search?semantic=true`, all off cleanly with no sidecar) ✅, and 3b (cheap, explainable auto-tagging — MIME categories + a keyword vocabulary, applied during extraction; user tags survive re-tagging) ✅, and 4 (OIDC single sign-on alongside passkeys — authorization-code + PKCE, opt-in, provisions its own non-admin users, never weakening the passkey paths) ✅; then a hardening pass |
