...
| 4 | ML: OCR, semantic search, tagging; OIDC; hardening | **design** — see [docs/phase-4-design.md](docs/phase-4-design.md): all intelligence runs in a separate, throttled, killable `pcworker` off a DB job queue, never in the API and never resident, because the always-on box is 7 GiB and one spinner; content never leaves for a hosted model. Slices: job queue → OCR into the existing search → embeddings + cheap tagging → OIDC → a hardening pass |
