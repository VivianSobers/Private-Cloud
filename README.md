...
| **3** | Sync engine: change journal, Go client, conflict resolution | **in progress** — slices 1–2 complete: a per-owner change journal with a gap-free cursor (`GET /changes`), and a block-level delta protocol (fetch manifest, negotiate missing chunks, BLAKE3-verified chunk upload, manifest commit) so a changed file moves only its changed chunks; Go client and conflict resolution next |
